...
Transpose all chords in the grid by a number of semitones (between -12 and +12).  
Enter the desired value in the semitone field and click `Apply`.  
The chord input fields become read-only while a transposition is active — set the value back to 0 to restore editing.

> 💡
> Transposition is non-destructive: the original chords are preserved and restored when you reset to 0.
